...
class MainWindow extends WindowBase_1.WindowBase {
    constructor(appConfig, logger, clientVersion, updater, localisation) {
        ...
        this._skypeUri = SkypeUriInit_1.getInstance();
        this._badgeCount = 0;
        this._appConfig = appConfig;
        this._localisation = localisation;
        this._logger = logger;
        this._dockBounceId = undefined;
        this._taskbarIconFlashed = false;
    ...
    static _getWindowOptions(appConfig, logger, localisation) {
        ...
        if (Settings_1.settings.has(Settings_1.SettingsKeys.WindowPosition)) {
            try {
                let savedPosition = Settings_1.settings.get(Settings_1.SettingsKeys.WindowPosition);
    ...
    _handleNotifications() {
        ...
        electron_1.ipcMain.on('flash-taskbar-icon', () => {
            if (this.hasValidWindow() && (!this.window.isFocused() || this.window.isMinimized()) && !this._taskbarIconFlashed) {
                this.window.flashFrame(true);
                if (electron.app.dock && this._dockBounceId) {
                    this._dockBounceId = electron.app.dock.bounce();
                    if (this._dockBounceId === -1) {
                        this._dockBounceId = undefined;
                    }
                }
                this._taskbarIconFlashed = true;
            }
        });
    ...
    _saveWindowPosition() {
        ...
        Settings_1.settings.set(Settings_1.SettingsKeys.WindowPosition, this.window.getBounds());
    }
    _handleWindowClose() {
